...
export default function EntranceScreen({
  ...
  useEffect(() => {
    if (isToken) return;
    if (!loading && !localEvent) navigate("/", { replace: true });
  ...
  const markTable = useCallback((tableId, on) => {
    if (!canWrite) return;
    if (isToken) {
      const rows = (remote?.guests || []).filter(g => remote.seating?.[g.id] === tableId);
  ...
  // ── One guest row ──────────────────────────────────────────────────────────
  const GuestRow = ({ g, matchLabel, compact, declined }) => {
        ...
        <>
          <div className={styles.searchWrap}>
            ...
            <input
              ...
              placeholder="שם האורח, שם מלווה או טלפון"
          ...
          {search && results.length === 0 && (
            <div className={styles.empty}>
              ...
              <p className={styles.emptyHint}>החיפוש עובר גם על שמות המלווים ועל מספרי טלפון</p>
              {canManage && (
          ...
          <div className={styles.tableList}>
                ...
                <div key={table.id} className={[styles.tableBlock, allIn ? styles.tableBlockDone : ""].filter(Boolean).join(" ")}>
                  ...
                  <div className={styles.tableGuests}>
                    {rows.length === 0
                      ...
                      : rows.map(g => <GuestRow key={g.id} g={g} compact />)}
                  </div>
                </div>
              );
            })}
            ...
            {!tq && unassigned.length > 0 && (
              <div className={styles.tableBlock}>
                ...
                <div className={styles.tableGuests}>
                  {unassigned.map(g => <GuestRow key={g.id} g={g} compact />)}
                </div>
              </div>
            )}
          </div>
            ...
            <div className={styles.linkBody}>
              ...
              <div className={styles.linkActions}>
                <button
                  ...
                  onClick={() => navigator.clipboard?.writeText(`${window.location.origin}/entrance/${ev.tokens?.hostess || ""}`)}
                >
