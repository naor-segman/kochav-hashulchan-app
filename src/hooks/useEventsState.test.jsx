...
// Tokens are part of the fixture, not decoration. Without them `normalizeEvent`
// mints a fresh uuid per missing token on EACH side of a merge, the two sets
// differ, and the merge correctly concludes the cloud does not hold this
// content — so a hydration that should be a no-op looked like a real change.
// Every real event has tokens; a fixture without them is not a smaller event,
// it is an impossible one.
const TOKENS = { rsvp: "t-rsvp", invite: "t-invite", gift: "t-gift",
                 album: "t-album", hostess: "t-hostess", collab: "t-collab" };

const ev = (id, over = {}) => ({
  id, name: "אירוע " + id, type: "חתונה", date: "2027-06-01", venue: "אולם",
  guests: [], tables: [], seating: {}, constraints: [], tokens: { ...TOKENS },
...
describe("useEvents — the conflict recovery pushes what it merged", () => {
  // THE OTHER HALF OF THE DATA-LOSS BUG. useEventsConflict.test.js pins that a
  // merged event is not mistaken for one the cloud holds. This pins that the
  // merge does not just sit there: before this, the recovery re-fetched, merged,
  // set SYNCED and STOPPED — it "resolved" a conflict by leaving the resolution
  // on one device. The cloud kept the pre-conflict copy indefinitely, because
  ...
  it("pushes the merged event a second time, against the fresh base", async () => {
    ...
    // First push conflicts; the retry after the merge succeeds and the server
    // reports the version it stored.
    cloud.updateCloudEvent
      .mockRejectedValueOnce(new cloud.CloudConflictError())
      .mockResolvedValueOnce(7);

    const { result } = renderHook(() => useEvents(USER));
    await settle();
    act(() => { result.current.patchEventById("e1", { venue: "היכל חדש יותר" }); });
    await flushDebounce();
    await settle();

    // Twice: the push that conflicted, then the push of the merge.
    expect(cloud.updateCloudEvent).toHaveBeenCalledTimes(2);
    const pushed = cloud.updateCloudEvent.mock.calls[1][0];
    expect(pushed.venue, "the local edit is what reaches the cloud").toBe("היכל חדש יותר");
    expect(pushed.syncedVersion, "the base is the row we just read").toBe(6);
    expect(pushed.version, "one past that base").toBe(7);
  });

  it("leaves the event dirty when the second push also fails", async () => {
    ...
    expect(cloud.updateCloudEvent).toHaveBeenCalledTimes(2);
    const e = stored(userKey("u1")).find(x => x.id === "e1");
    expect(e.version).not.toBe(e.syncedVersion);
  });

  it("always retries after a conflict — a rejected push has not landed", async () => {
    // Deliberately unconditional. Deciding by comparing the merged event to the
    // cloud row was tried and abandoned: normalizeEvent mints a fresh uuid for
    // every event-site FAQ row and every missing token, so two copies of
    // identical content never serialise the same. A conflict is rare; one extra
    // write when the contents happen to match is the cheap side of that trade.
    seed(userKey("u1"), [ev("e1", { cloudId: "c1", version: 6, syncedVersion: 5, updatedAt: 1_000 })]);
    cloud.fetchCloudEvents.mockImplementation(async () => [
      { ...ev("e1", { cloudId: "c1", version: 6, syncedVersion: 6, updatedAt: 1_000 }) },
    ]);
    cloud.updateCloudEvent
      .mockRejectedValueOnce(new cloud.CloudConflictError())
      .mockResolvedValueOnce(7);
    ...
    const { result } = renderHook(() => useEvents(USER));
    await settle();
    act(() => { result.current.patchEventById("e1", { venue: "היכל" }); });
    await flushDebounce();
    await settle();

    expect(cloud.updateCloudEvent).toHaveBeenCalledTimes(2);
    // And the event ends up genuinely backed, not merely equal by accident.
    const e = stored(userKey("u1")).find(x => x.id === "e1");
    expect(e.syncedVersion).toBe(7);
